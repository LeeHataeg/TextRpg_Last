...
namespace TextRpg_Test01
{
    internal class Program
    {
        ...
        public static List<Potion> shopPotion = new List<Potion>();
        //장비창(10칸)
        public static List<Item> myInven = new List<Item>(10);
        public static List<Potion>myPotion = new List<Potion>(10);
        //장비 효과 수치
        ...
        public static void GameDataSetting()
        {
            ...
            shopPotion.Add(lowHpPotion);
            shopPotion.Add(lowAtkPotion);
            shopPotion.Add(lowDefPotion);
        }
        ...
        static void DisplayInventory()
        {
            ...
            int itemLength = myInven.Count;
            int currentItemIndex = 0;

            while (currentItemIndex < itemLength)
            {
                ...
                if (myInven[currentItemIndex].Item_Atk == 0)
                {
                    //armor
                    int effectLength = HowManyDigit(myInven[currentItemIndex].Item_Def) + 6;
                    Replace_Effect = InvenStr_Effect.Remove(0, effectLength)
                                                    .Insert(0, " 방어력 +" + Convert.ToString(myInven[currentItemIndex].Item_Def));
                }
                else
                {
                    //weapon
                    int effectLength = HowManyDigit(myInven[currentItemIndex].Item_Atk) + 6;
                    Replace_Effect = InvenStr_Effect.Remove(0, effectLength)
                                                    .Insert(0, " 공격력 +" + Convert.ToString(myInven[currentItemIndex].Item_Atk));
                }


                int explainLength = myInven[currentItemIndex].Item_Discription.Length;
                Replace_Explain = InvenStr_Explain.Remove(0, explainLength)
                                                  .Insert(0, myInven[currentItemIndex].Item_Discription);
            ...
            int potionLength = myPotion.Count();
            int currentPotionIndex = 0;

            while (currentPotionIndex < potionLength)
            {
                string InvenStr_Name = "          |";
                string InvenStr_Effect = "          |";
                string InvenStr_Explain = "                                                            |";
                ...
                int potionnameLength = myPotion[currentPotionIndex].Item_Name.Length;
                Replace_Name = InvenStr_Name.Remove(0, potionnameLength)
                                            .Insert(0, myPotion[currentPotionIndex].Item_Name);



                //몇 자리 숫자인지(X / 10  +1(1의 자리는 10으로 나눴을 때 0)) +6(" 공격력 +"/" 방어력 +" 등 텍스트)
                if (myPotion[currentPotionIndex].Potion_Healing > 0)
                {
                    int potioneffectLength = HowManyDigit(myPotion[currentPotionIndex].Potion_Healing) + 5;
                    Replace_Effect = InvenStr_Effect.Remove(0, potioneffectLength)
                                                    .Insert(0, " 힐량 +" + Convert.ToString(myPotion[currentPotionIndex].Potion_Healing));
                }
                else if (myPotion[currentPotionIndex].Item_Atk > 0)
                {
                    //armor
                    int potioneffectLength = HowManyDigit(myPotion[currentPotionIndex].Item_Atk) + 6;
                    Replace_Effect = InvenStr_Effect.Remove(0, potioneffectLength)
                                                    .Insert(0, " 방어력 +" + Convert.ToString(myPotion[currentPotionIndex].Item_Atk));
                }
                else if (myPotion[currentPotionIndex].Item_Def > 0)
                {
                    //weapon
                    int potioneffectLength = HowManyDigit(myPotion[currentPotionIndex].Item_Def) + 6;
                    Replace_Effect = InvenStr_Effect.Remove(0, potioneffectLength)
                                                    .Insert(0, " 공격력 +" + Convert.ToString(myPotion[currentPotionIndex].Item_Def));
                }

                int potionexplainLength = myPotion[currentPotionIndex].Item_Discription.Count();
                Replace_Explain = InvenStr_Explain.Remove(0, potionexplainLength)
                                                  .Insert(0, myPotion[currentPotionIndex].Item_Discription);
        ...
        public static void ShopBuy()
        {
            ...
            int shopLength = shopItem.Count();
            int currentShopIndex = 0;

            while (currentShopIndex < shopLength)
            {
                Console.Write("- ");
                Console.ForegroundColor = ConsoleColor.Magenta;
                Console.Write($"{currentShopIndex + 1} ");
                Console.ForegroundColor = ConsoleColor.White;
                Console.Write(shopItem[currentShopIndex].Item_Name);
                Console.ForegroundColor = ConsoleColor.White;
                if (shopItem[currentShopIndex].Item_Atk == 0)
                {
                    //if Item == armor
                    Console.Write("    | 방어력 ");
                    Console.ForegroundColor = ConsoleColor.Magenta;
                    Console.Write(shopItem[currentShopIndex].Item_Def);
                }
                else
                {
                    //if item == weapon
                    Console.Write("    | 공격력 ");
                    Console.ForegroundColor = ConsoleColor.Magenta;
                    Console.Write(shopItem[currentShopIndex].Item_Atk);
                }
                Console.ForegroundColor = ConsoleColor.White;
                Console.Write(" | 가격 ");
                Console.ForegroundColor = ConsoleColor.Magenta;
                Console.Write(shopItem[currentShopIndex].Item_Gold);
                Console.ForegroundColor = ConsoleColor.White;
                Console.WriteLine($"G | {shopItem[currentShopIndex].Item_Discription}");
                
                currentShopIndex++;
            }
            int potionShopLength = shopPotion.Count();
            int PotionShopIndex = 0;

            while (PotionShopIndex < potionShopLength)
            {
                Console.Write("- ");
                Console.ForegroundColor = ConsoleColor.Magenta;
                Console.Write($"{shopLength + PotionShopIndex +1} ");
                Console.ForegroundColor = ConsoleColor.White;
                Console.Write(shopPotion[PotionShopIndex].Item_Name);
                Console.ForegroundColor = ConsoleColor.White;

                if (shopPotion[PotionShopIndex].Potion_Healing > 0)
                {
                    Console.Write("    | 회복량 +");
                    Console.ForegroundColor = ConsoleColor.Magenta;
                    Console.Write(shopPotion[PotionShopIndex].Potion_Healing);
                }
                else if(shopPotion[PotionShopIndex].Item_Atk > 0)
                {
                    Console.Write("    | 공격력 증가 +");
                    Console.ForegroundColor = ConsoleColor.Magenta;
                    Console.Write(shopPotion[PotionShopIndex].Item_Atk);
                }
                else if(shopPotion[PotionShopIndex].Item_Def > 0)
                {
                    Console.Write("    | 방어력 증가 +");
                    Console.ForegroundColor = ConsoleColor.Magenta;
                    Console.Write(shopPotion[PotionShopIndex].Item_Def);
                }

                Console.ForegroundColor = ConsoleColor.White;
                Console.Write(" | 가격 ");
                Console.ForegroundColor = ConsoleColor.Magenta;
                Console.Write(shopPotion[PotionShopIndex].Item_Gold);
                Console.ForegroundColor = ConsoleColor.White;
                Console.WriteLine($"G | {shopPotion[PotionShopIndex].Item_Discription}");
            ...
            int input = CheckValidInput(0, shopLength + potionShopLength);
            if(input == 0)
            {
                DisplayShop();
            }
            else if(input <= shopLength)
            {
            ...
            {
                if (player.Gold < shopPotion[input -shopLength -1].Item_Gold)
                {
                    Console.WriteLine("소지하신 골드가 적습니다.");
                    ShopBuy();
                }
                ...
                {
                    Console.WriteLine("구매를 완료했습니다.");
                    player.Gold -= shopPotion[input - shopLength - 1].Item_Gold;
                    myInven.Add(shopPotion[input - shopLength - 1]);
                    //shopItem.RemoveAt();
                    ShopBuy();
                }
            }
        ...
        public static void ShopSell()
        {
            ...
            while (currentItemIndex < itemLength)
            {
                ...
                int goldLength = HowManyDigit(myInven[currentItemIndex].Item_Gold) + 6;
                Replace_Gold = InvenStr_Gold.Remove(0, goldLength)
                                            .Insert(0, "가격 : " + Convert.ToString(myInven[currentItemIndex].Item_Gold * 85 / 100) + "G");

                int explainLength = myInven[currentItemIndex].Item_Discription.Count();
                Replace_Explain = InvenStr_Explain.Remove(0, explainLength)
                                                  .Insert(0, myInven[currentItemIndex].Item_Discription);

                Console.Write($"- ");
                Console.ForegroundColor = ConsoleColor.Magenta;
                Console.Write($"{currentItemIndex + 1} ");
                Console.ForegroundColor = ConsoleColor.White;
                Console.Write(Replace_Name);
                Console.Write(Replace_Effect);
            ...
            while (currentPotionIndex < potionLength)
            {
                ...
                int nameLength = myPotion[currentPotionIndex].Item_Name.Count();
                Replace_Name = InvenStr_Name.Remove(0, nameLength)
                                            .Insert(0, myPotion[currentPotionIndex].Item_Name);
                ...
                if (myPotion[currentPotionIndex].Potion_Healing > 0)
                {
                    int effectLength = HowManyDigit(myPotion[currentPotionIndex].Potion_Healing) + 5;
                    Replace_Effect = InvenStr_Effect.Remove(0, effectLength)
                                                    .Insert(0, " 힐량 +" + Convert.ToString(myPotion[currentPotionIndex].Potion_Healing));
                }
                else if (myPotion[currentPotionIndex].Item_Atk > 0)
                {
                    //armor
                    int effectLength = HowManyDigit(myPotion[currentPotionIndex].Item_Atk) + 6;
                    Replace_Effect = InvenStr_Effect.Remove(0, effectLength)
                                                    .Insert(0, " 방어력 +" + Convert.ToString(myPotion[currentPotionIndex].Item_Atk));
                }
                else if (myPotion[currentPotionIndex].Item_Def > 0)
                {
                    //weapon
                    int effectLength = HowManyDigit(myPotion[currentPotionIndex].Item_Def) + 6;
                    Replace_Effect = InvenStr_Effect.Remove(0, effectLength)
                                                    .Insert(0, " 공격력 +" + Convert.ToString(myPotion[currentPotionIndex].Item_Def));
                }

                int goldLength = HowManyDigit(myPotion[currentPotionIndex].Item_Gold * 85 / 100) + 6;
                Replace_Gold = InvenStr_Gold.Remove(0, goldLength)
                                            .Insert(0, "가격 : " + Convert.ToString(myPotion[currentPotionIndex].Item_Gold / 100 * 85) + "G");

                int explainLength = myPotion[currentPotionIndex].Item_Discription.Count();
                Replace_Explain = InvenStr_Explain.Remove(0, explainLength)
                                                  .Insert(0, myPotion[currentPotionIndex].Item_Discription);

                Console.Write($"- ");
                Console.ForegroundColor = ConsoleColor.Magenta;
                Console.Write($"{itemLength + currentPotionIndex + 1} ");
                Console.ForegroundColor = ConsoleColor.White;
                Console.Write(Replace_Name);
                Console.Write(Replace_Effect);
                Console.Write(Replace_Gold);
            ...
            int input = CheckValidInput(0, itemLength + potionLength);
            if(input == 0)
            {
                DisplayShop();
            }
            else if(input <= itemLength)
            {
                Console.WriteLine("판매를 완료했습니다.");
                player.Gold += (myInven[input - 1].Item_Gold / 100) * 85;
            ...
            {
                Console.WriteLine("판매를 완료했습니다.");
                player.Gold += myPotion[input - itemLength - 1].Item_Gold * 85 / 100;
                myPotion.RemoveAt(input - itemLength - 1);
                DisplayShop();
            }
        }
